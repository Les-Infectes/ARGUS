...
def generate_class_c_router_targets(current_cidr):
    """Génère toutes les IP .1 et .254 pour les sous-réseaux 192.168.x.x sauf celui en cours."""
    targets = set()
    try:
        current_network = ipaddress.ip_network(current_cidr, strict=False)
    except ValueError:
        return []

    for subnet_id in range(0, 256):
        candidate_net = ipaddress.ip_network(f"192.168.{subnet_id}.0/24")
        if current_network.overlaps(candidate_net):
            continue
        for host in (1, 254):
            targets.add(f"192.168.{subnet_id}.{host}")
...
def run_nmap_port_scan(hosts, port_list=None):
    ...
    # Arguments uniformisés : -Pn -A -T4 (ordre cohérent avec script classeAB)
...
def main():
    parser = argparse.ArgumentParser(
        description="Scan global (VLAN ARP, traceroute, brute-force routers classe C et scan /24)."
    )
    ...
    report = {
        "summary": "Scan combiné VLAN ARP / traceroutes / brute-force routeurs classe C / scan de sous-réseaux.",
        "configuration": {
    ...
    print("\n" + "=" * 70)
    print("## Scan global : VLAN + Trace + Routeurs classe C")
    print("=" * 70)
    ...
    class_c_targets = generate_class_c_router_targets(args.ip_cidr)
    combined_candidates = set(class_c_targets) | discovered_routers
    report["router_discovery"]["candidates"] = sorted(combined_candidates)

    # Phase 3 simplifiée: Vérification ICMP directe (nmap -sn redondant supprimé)
    icmp_verified = confirm_with_icmp(sorted(combined_candidates))
